...
def plot_persistent_train_val_test_for(ibuc, model_persistent, device, var=target_col):
    """Save plots for a single basin (Train/Val/Test) into a folder."""
    ...
    fig.suptitle(f'{var} — Train/Val/Test — Basin {ibuc}', y=0.98)
    plt.tight_layout()

    # ----------------------------------------------------
    # Save plot to file instead of showing it
    # ----------------------------------------------------
    save_path = f"plots/basin_{ibuc}_persistent.png"
    plt.savefig(save_path, dpi=300)
    plt.close(fig)

    print(f"Saved plot for basin {ibuc} → {save_path}")
